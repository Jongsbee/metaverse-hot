...
public class MissileForPhoton : MonoBehaviour
{
    Rigidbody rigidBody = null;
    [SerializeField] private Transform thisTarget = null;
    EnemyForPhoton targetEnemy;
    float thisDamage;
    ...
    void Start()
    {
        ...
        targetEnemy = thisTarget.GetComponent<EnemyForPhoton>();
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        {
            Destroy(gameObject);
            EnemyForPhoton enemy = collision.gameObject.GetComponent<EnemyForPhoton>();
            if (thisDeBuff > 0)
            {
                DeBuffAttack(enemy);
            }
            else
            {
                ...
                enemy.photonView.RPC("ProcessHitWithNoParticleNoDeath",RpcTarget.Others,thisDamage);
            }
        }
    }
